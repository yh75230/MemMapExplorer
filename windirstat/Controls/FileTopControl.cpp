...
void CFileTopControl::SortItems()
{
    ...
    // Remove items that were deleted before we touch cached pointers again.
    if (!m_removedItems.empty())
    {
        std::erase_if(m_sizeMap, [this](const auto& cachedItem)
        {
            return m_removedItems.contains(cachedItem);
        });
    }
    ...
    {
        if (m_removedItems.contains(newItem))
        {
            continue;
        }
...
void CFileTopControl::RemoveItem(CItem* item)
{
    ...
    {
        // Create list of all items to remove while the subtree is still alive.
        ...
        // Mark removed items first so queued stale pointers are ignored.
        m_removedItems.insert(toRemove.begin(), toRemove.end());

        // Remove items in bulk from the cached size list.
        m_needsResort = true;
